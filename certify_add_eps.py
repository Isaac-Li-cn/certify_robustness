...
def change_data(data_batch, change):
    '''
    >>> data_batch: tensor of shape [batch_size, in_dim]
    >>> change: tensor of shape [batch_size, in_dim]
    >>> 如果data_batch为正，就减去；如果为负，就加上
    >>> 初始实现，只考虑mnist数据集，因此为10和784
    >>> 考虑syn数据集，为10和2 2020/08/04
    '''
    for i in range(10):
        for j in range(2):
            if (data_batch[i][j] > 0):
                data_batch[i][j] = data_batch[i][j] - change[i][j]
            else:
                data_batch[i][j] = data_batch[i][j] + change[i][j]
    return data_batch
...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_eps1', type = str, default = None,
        help = 'the file including eps1 if needed')
    parser.add_argument('--add_eps2', type = str, default = None,
        help = 'the file including eps2 if needed')
    parser.add_argument('--add_eps3', type = str, default = None,
        help = 'the file including eps3 if needed')
    parser.add_argument('--add_eps4', type = str, default = None,
        help = 'the file including eps4 if needed')
    parser.add_argument('--add_eps5', type = str, default = None,
        help = 'the file including eps5 if needed')
    parser.add_argument('--add_eps6', type = str, default = None,
        help = 'the file including eps6 if needed')
    parser.add_argument('--add_eps7', type = str, default = None,
        help = 'the file including eps7 if needed')
    parser.add_argument('--add_eps8', type = str, default = None,
        help = 'the file including eps8 if needed')
    parser.add_argument('--add_eps9', type = str, default = None,
        help = 'the file including eps9 if needed')
    parser.add_argument('--add_eps10', type = str, default = None,
        help = 'the file including eps10 if needed')
    ...
    for batch_idx in range(args.batch_num):
        ...
        # 改变到临界值，首先导入文件
        if args.add_eps1 is not None:
            file_data = pickle.load(open(args.add_eps1, 'rb'))
        ...
        if args.add_eps2 is not None:
            file_data = pickle.load(open(args.add_eps2, 'rb'))
            results_old = file_data['results']
            # print(results_old)
            change = results_old[batch_idx]['eps']
            # print(change)
        ...
        if args.add_eps3 is not None:
            file_data = pickle.load(open(args.add_eps3, 'rb'))
            results_old = file_data['results']
            # print(results_old)
            change = results_old[batch_idx]['eps']
            # print(change)
        ...
        if args.add_eps4 is not None:
            file_data = pickle.load(open(args.add_eps4, 'rb'))
            results_old = file_data['results']
            # print(results_old)
            change = results_old[batch_idx]['eps']
            # print(change)
        ...
        if args.add_eps5 is not None:
            file_data = pickle.load(open(args.add_eps5, 'rb'))
            results_old = file_data['results']
            # print(results_old)
            change = results_old[batch_idx]['eps']
            # print(change)
        ...
        if args.add_eps6 is not None:
            file_data = pickle.load(open(args.add_eps6, 'rb'))
            results_old = file_data['results']
            # print(results_old)
            change = results_old[batch_idx]['eps']
            # print(change)
        ...
        if args.add_eps7 is not None:
            file_data = pickle.load(open(args.add_eps7, 'rb'))
            results_old = file_data['results']
            # print(results_old)
            change = results_old[batch_idx]['eps']
            # print(change)
